...
if(isset($_POST['carrier'])){
	 ...
	 print("</div><div class='row'><div class='col strike'>
   <span>$carrier </span>
</div></div>
	
	 <div  class='row'>");
	  $query_carriers = "Select id_carrier From data_carriers Where Name='$carrier'";
           $result_carriers = mysqli_query($link,$query_carriers) or die("Ошибка".mysqli_error($link));
		   if($result_carriers)
           {
           	
               while($row = mysqli_fetch_assoc($result_carriers))
               {
                  $dcarrier = $row['id_carrier']; 
                  
               }
			   
           }
	  $query="select * from product where id_carrier = '$dcarrier' ";
}
else{
