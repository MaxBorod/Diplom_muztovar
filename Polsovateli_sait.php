...
<?php session_start(); ?>
<head>
<title>Пользователи сайта</title><script src="js/script.js"></script>
...
$query_pols="select * from klient ";
$result_pols = mysqli_query($link,$query_pols);
print ("
...
if($result_pols)
{
    while($row = mysqli_fetch_assoc($result_pols))
    {
       $id= $row['id']; 
       $Surname= $row['Surname']; 
	   $Name= $row['Name'];
...
<div class="strike">
   <span>Изменить роль пользователя</span>
</div>
<form action="change_rol.php" method="post">
	<div class='p-1'>
	 <label class=''>Введите id_пользователя:</label>
	<input type="text" name='id_pols'/>
	 <select  name="roleOption">
  <option value="1">Администратор</option>
  <option value="2">Пользователь</option>
</select>
<input type="submit"  class="btn btn-warning" value='Изменить роль'/>
	</div>
